...
### Testing
- All external integrations sit behind adapters with a mock implementation
- The agent can validate everything including the Slack interface via mocks
- Blackbox scenarios map directly to stories in the tree
- Each slice is independently validatable before the next starts

## NFRs

- **Crash safety** — no work is ever lost. An effect must be persisted before a worker picks it up.
- **Testability** — every integration is mockable. The agent can run full blackbox scenarios without external dependencies.
...
- **Iterative delivery** — every slice is independently deployable and blackbox-testable before the next starts.
- **Golden path** — before any pattern is implemented, a canonical example must exist. Logging, tracing, DB migrations, CLI flags, error handling, testing — all defined upfront. If the golden path for something doesn't exist, design it with the user before building.
